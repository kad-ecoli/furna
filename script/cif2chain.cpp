    ...
    const string &chainID)
{
    ...
    map<string,string> label_asym_id2entity_id;
    ...
    for (l=0;l<lines.size();l++)
    {
        ...
        else if (line_vec.size()==1 && line_vec[0]=="#")
        {
            ...
            if (_struct_asym_id.size() && _struct_asym_entity_id.size())
            {
                label_asym_id2entity_id[_struct_asym_id]=_struct_asym_entity_id;
                _struct_asym_id.clear();
                _struct_asym_entity_id.clear();
            }
            if (_entity_src_gen_entity_id.size() && _entity_src_gen_taxon.size())
            {
                entity_id2taxon[_entity_src_gen_entity_id]=_entity_src_gen_taxon;
        ...
        else if (StartsWith(line,"_struct_asym."))
        {
            if (loop_)
            {
                line=line_vec[0];
            ...
            {
                l=read_semi_colon(line_vec, 2, l, lines, line_append_vec, line,false,true);
                if      (line_vec[0]=="_struct_asym.id")
                    _struct_asym_id=Trim(line_vec[1],"\"");
                else if (line_vec[0]=="_struct_asym.entity_id")
                    _struct_asym_entity_id=Trim(line_vec[1],"\"");
            }
        }
        else if (_struct_asym.size() &&
                 _struct_asym.count("entity_id") && _struct_asym.count("id"))
        {
            l=read_semi_colon(line_vec, _struct_asym.size(),
                l, lines, line_append_vec, line,false,true);
            _struct_asym_id=Trim(line_vec[_struct_asym["id"]],"\"");
            _struct_asym_entity_id=Trim(line_vec[_struct_asym["entity_id"]],"\"");
            label_asym_id2entity_id[_struct_asym_id]=_struct_asym_entity_id;
            _struct_asym_id.clear();
            _struct_asym_entity_id.clear();
        }
            ...
            _atom_site.count("Cartn_y") && _atom_site.count("Cartn_z"))
        {
            ...
            if (_atom_site.count("auth_asym_id") && _atom_site.count("label_asym_id") &&
                auth_asym_id2label_asym_id.count(line_vec[_atom_site["auth_asym_id"]])==0)
            {
                auth_asym_id2label_asym_id[line_vec[_atom_site["auth_asym_id"]]]=
                                           line_vec[_atom_site["label_asym_id"]];
            }
    ...
    if (auth_asym_id2label_asym_id.count(chainID))
    {
        string label_asym_id=auth_asym_id2label_asym_id[chainID];
        if (label_asym_id2entity_id.count(label_asym_id))
        {
