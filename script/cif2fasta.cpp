...
"cif2fasta folder list\n"
"    convert a folder of cif.gz files into fasta sequences\n"
"\n"
"input:\n"
...
int cif2fasta(const string &infile, string &pdbid)
{
    ...
    string pdbx_PDB_model_num_prev="";
    ...
    for (l=0;l<lines.size();l++)
    {
        ...
        else if (_atom_site.size())
        {
            if (_atom_site.count("pdbx_PDB_model_num"))
            {
                ...
                {
                    if (pdbx_PDB_model_num_prev.size() && 
...
void readListFile(string &listfile, vector<string> &pdbid_list)
{
    stringstream buf;
    if (listfile=="-") buf<<cin.rdbuf();
#if defined(REDI_PSTREAM_H_SEEN)
    else if (EndsWith(listfile,".gz"))
    {
        redi::ipstream fp_gz; // if file is compressed
        fp_gz.open("gunzip -c "+listfile);
        buf<<fp_gz.rdbuf();
        fp_gz.close();
    }
#endif
    ...
    {
        ifstream fp;
        fp.open(listfile.c_str(),ios::in); //ifstream fp(filename,ios::in);
        buf<<fp.rdbuf();
        fp.close();
    }
    Split(buf.str(),pdbid_list,'\n',true); 
}
...
int main(int argc,char **argv)
{
    ...
    vector<string> pdbid_list;
    readListFile(listfile,pdbid_list);

    size_t i;
    if (folder[folder.size()-1]!='/' && 
        folder[folder.size()-1]!='\\') folder+="/"; 
    string pdbid;
    string filename;
    for (i=0;i<pdbid_list.size();i++)
    {
        pdbid=pdbid_list[i];
        filename=folder+pdbid+".cif.gz";
        if (!isfile(filename)) filename=folder+pdbid.substr(
            pdbid.size()-3,2)+'/'+pdbid+".cif.gz";
        if (isfile(filename)) cif2fasta(filename,pdbid);
        else cerr<<"miss "+filename<<endl;
    }
    ...
    /* clean up */
    string ().swap(folder);
    string ().swap(listfile);
    vector<string> ().swap(pdbid_list);
    string ().swap(pdbid);
    string ().swap(filename);
    return 0;
}
