...
&download_from_UniChem("src_id1/src1src3.txt.gz","UniChem/src1src3.txt.gz");
&download_from_UniChem("src_id2/src2src3.txt.gz","UniChem/src2src3.txt.gz");
&download_from_UniChem("src_id3/src3src7.txt.gz","UniChem/src3src7.txt.gz");
&download_from_UniChem("src_id3/src3src9.txt.gz","UniChem/src3src9.txt.gz");
my %pdb2chembl;
...
for (my $l=0;$l<scalar @lines;$l++)
{
    ...
    if ($line=~/^data_/)
    {
        if (length $_chem_comp_id)
        {
            ...
            if (exists($interaction_dict{$_chem_comp_id}))
            {
...
sub download_from_UniChem
{
    my ($url_query,$outfile)=@_;
    system("wget -q  ftp://ftp.ebi.ac.uk/pub/databases/chembl/UniChem/data/wholeSourceMapping/$url_query -O $rootdir/$outfile");
    system("wget -q http://ftp.ebi.ac.uk/pub/databases/chembl/UniChem/data/wholeSourceMapping/$url_query -O $rootdir/$outfile") if (!-s "$rootdir/$outfile");
}
