...
rcl  =form.getfirst("rnacentral",'').upper().strip().strip("'")
rfm  =form.getfirst("rfam",'').upper().strip().strip("'")
...
if rfm:
    para_list.append("rfm=%s"%rcl)
if txn:
    para_list.append("txn=%s"%rcl)
if ecn:
...
#for line in stdout.decode().splitlines():
    ...
    if got and not go_mf+go_bp+go_cc:
        continue
    if ecn and not ec:
        continue
